...
const getE164 = async(number, country) => {
  // Quitar el signo + si existe
  const cleanNumber = number.replace('+', '');
  
  // Si empieza con 549, quitar el 9
  const processedNumber = cleanNumber.startsWith('549') ? 
    `54${cleanNumber.substring(3)}` : 
    cleanNumber;
  
  if (processedNumber.startsWith('5411')) {
    // Si empieza con 5411, agregar 15 después del 11
    return `541115${processedNumber.substring(4)}`;
  } else {
    // Para otros números, tomar los primeros 5 dígitos, agregar 15 y el resto
    const firstFive = processedNumber.substring(0, 5);
    const rest = processedNumber.substring(5);
    return `${firstFive}15${rest}`;
  }
}
