...
const getE164 = async(number, country) => {
  ...
  if (cleanNumber.startsWith('5411')) {
    // Si empieza con 5411, agregar 15 después del 11
    return `541115${cleanNumber.substring(4)}`;
  } else if (cleanNumber.startsWith('549')) {
    // Si empieza con 549, quitar el 9 y agregar 15 después del 54
    return `543415${cleanNumber.substring(3)}`;
  } else {
    // Para otros números, tomar los primeros 5 dígitos, agregar 15 y el resto
    return `${cleanNumber.substring(0, 5)}15${cleanNumber.substring(5)}`;
  }
}
